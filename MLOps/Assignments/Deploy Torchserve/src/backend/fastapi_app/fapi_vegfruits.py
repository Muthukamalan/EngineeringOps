...
class Sportprediction:
    def load_model(self) -> None:
        self.session = ort.InferenceSession(
            os.path.join("checkpoints", "onnxs", "vegfruits.onnx")
        )
    ...
    def predict(self, image: Image.Image):
        ...
        outputs = self.session.run(
            None, {self.session_input_name: img.astype(np.float32)}
        )
        logits = outputs[0][0]
        ...
        return {"confidence": float(confidence), "label": str(predicted_label)}
