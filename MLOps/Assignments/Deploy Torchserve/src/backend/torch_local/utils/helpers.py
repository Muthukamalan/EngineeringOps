...
def custom_loader(path: str, *, size: tuple = (224, 224)) -> NDArray:
    ...
    )  # .transpose(0,1,2) #,dtype=np.float32
    ...
    for batch in dataloader:
        for _class in batch[1]:
            if _class.item() not in class_distribution:
                ...
                class_distribution[_class.item()] += 1
    ax = (
        pd.DataFrame(index=class_distribution.keys(), data=class_distribution.values())
        .reset_index()
        .rename(columns={0: "distribution"})
